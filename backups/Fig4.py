...
results_dir = "results"
input_type = "cos"  # "impulse", "step", or "cos"
fs=100
dt = 1/fs
Trials = 10 # 5000 in the paper
inputTrials = 10 # 50 in the paper
    ...
    for trial in range(Trials):
        ...
        for trial in range(inputTrials):
            ...
            if test_on: 
                ...
                for d in range(dim):
                    plt.plot(np.arange(0, T, dt), x[d, :], label=f'Dimension {d+1}')
                plt.xlabel('Time [s]')
                plt.ylabel('State')
                plt.title('Time Series of X, '+str(value))
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
                # Save the time series plot as an SVG file
                time_series_filename = os.path.join(results_dir, f"{__file__.split('/')[-1].split('.')[0]}_time_series_{value}.svg")
                plt.savefig(time_series_filename)
                print(f"Time series plot saved as {time_series_filename}")
                plt.show()
                # Plot heatmaps of X@X.T, X_@X_.T, and U@U.T
                fig, axs = plt.subplots(1, 3, figsize=(20, 5))

                # Heatmap of X@X.T
                axs[0].imshow(Z @ Z.T, cmap='viridis', interpolation='nearest')
                axs[0].set_title('Z @ Z.T')
                axs[0].set_xlabel('Dimension')
                axs[0].set_ylabel('Dimension')
                for i in range(Z.shape[0]):
                    for j in range(Z.shape[0]):
                        axs[0].text(j, i, f"{(Z @ Z.T)[i, j]:.2f}", ha='center', va='center', color='black')

                # Heatmap of X_@X_.T
                axs[1].imshow(X @ X.T, cmap='viridis', interpolation='nearest')
                axs[1].set_title('X @ X.T')
                axs[1].set_xlabel('Dimension')
                axs[1].set_ylabel('Dimension')
                for i in range(X.shape[0]):
                    for j in range(X.shape[0]):
                        axs[1].text(j, i, f"{(X @ X.T)[i, j]:.2f}", ha='center', va='center', color='black')

                # Heatmap of U@U.T
                axs[2].imshow(U @ U.T, cmap='viridis', interpolation='nearest')
                axs[2].set_title('U @ U.T')
                axs[2].set_xlabel('Dimension')
                axs[2].set_ylabel('Dimension')
                for i in range(U.shape[0]):
                    for j in range(U.shape[0]):
                        axs[2].text(j, i, f"{(U @ U.T)[i, j]:.2f}", ha='center', va='center', color='black')
                        
                plt.suptitle(str(value))
                plt.tight_layout()
                plt.show()
                
                print(value, np.linalg.cond(Z@Z.T))
...
if input_type=="cos":
    input_values = np.arange(0.1, 50, 0.1)
else:
