...
heatmap_filename = os.path.join(results_dir, f"{__file__.split('/')[-1].split('.')[0]}_A_discrete_heatmap_dim{dim}.pdf")
...
graph_filename = os.path.join(results_dir, f"{__file__.split('/')[-1].split('.')[0]}_directed_graph.pdf")
...
for input_type in ["passive", "impulse", "cos"]:
    ...
    time_series_filename = os.path.join(results_dir, f"{__file__.split('/')[-1].split('.')[0]}_time_series_{input_type}.pdf")
    ...
    pca_filename = os.path.join(results_dir, f"{__file__.split('/')[-1].split('.')[0]}_PCA_X_{input_type}_{X.shape[1]}.pdf")
    ...
    # Save the heatmap as an SVG file
    heatmap_filename = os.path.join(results_dir, f"{__file__.split('/')[-1].split('.')[0]}_A_est_heatmap_{input_type}_dim{dim}.pdf")
    ...
    # Save the heatmap as an SVG file
    heatmap_filename = os.path.join(results_dir, f"{__file__.split('/')[-1].split('.')[0]}_A_error_heatmap_{input_type}_dim{dim}.pdf")
    plt.savefig(heatmap_filename, bbox_inches='tight', pad_inches=0.05)
    print(f"Heatmap saved as {heatmap_filename}")
