...
for fIndex, f in enumerate(mode_freq):
    ...
    # desired_eigenvalues.append([-0-(2*np.pi*f)*1j, -0+(2*np.pi*f)*1j])
...
values = [-0.03, -0.02, -0.01, 0, 0.01, 0.02, 0.03]
A_discrete[0,2] = -0.01
A_discrete[1,3] = 0.01
A_discrete[2,0] = 0.01
A_discrete[3,0] = -0.01
...
def compute_metrics(value, Trials, dim, fs, T, dt, A_true, B_true, input_type, test_on = False):
    ...
    for trial in range(Trials):
        ...
        for trial in range(inputTrials):
            ...
            u0[np.random.choice(dim)] = np.random.rand()
...
if input_type == "cos":
    axs[axs_index].set_xticks(np.arange(min(input_values)-0.1, max(input_values) + 1, 10))
...
if input_type == "impulse":
    ...
    axs[axs_index].set_xlabel('Frequency [Hz]')
axs[axs_index].set_ylabel(r'tr$((ZZ^{\text{T}})^{-1})$')
axs[axs_index].grid(True)
if input_type == "cos":
    axs[axs_index].set_xticks(np.arange(min(input_values)-0.1, max(input_values) + 1, 10))
    for i, base_f in enumerate(mode_freq):
